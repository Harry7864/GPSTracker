...
abstract class RxLocationSingleOnSubscribe<T> extends RxLocationBaseOnSubscribe<T> implements SingleOnSubscribe<T> {
    protected RxLocationSingleOnSubscribe( RxLocation rxLocation, Long timeout, TimeUnit timeUnit) {
        super(rxLocation, timeout, timeUnit);
    }

    protected RxLocationSingleOnSubscribe( Context ctx,  Api<? extends Api.ApiOptions.NotRequiredOptions>[] services, Scope[] scopes) {
