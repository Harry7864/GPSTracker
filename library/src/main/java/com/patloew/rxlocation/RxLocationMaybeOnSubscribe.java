...
abstract class RxLocationMaybeOnSubscribe<T> extends RxLocationBaseOnSubscribe<T> implements MaybeOnSubscribe<T> {
    protected RxLocationMaybeOnSubscribe( RxLocation rxLocation, Long timeout, TimeUnit timeUnit) {
        super(rxLocation, timeout, timeUnit);
    }

    protected RxLocationMaybeOnSubscribe( Context ctx,  Api<? extends Api.ApiOptions.NotRequiredOptions>[] services, Scope[] scopes) {
