...
class SingleResultCallBack<T extends Result, R> implements ResultCallback<T> {
    ...
    private SingleResultCallBack( SingleEmitter<R> emitter,  Function<T, R> mapper) {
        this.emitter = emitter;
        this.mapper = mapper;
    }

    static <T extends Result, R> ResultCallback<T> get( SingleEmitter<R> emitter,  Function<T, R> mapper) {
        return new SingleResultCallBack<>(emitter, mapper);
    }

    static <T extends Result> ResultCallback<T> get( SingleEmitter<T> emitter) {
