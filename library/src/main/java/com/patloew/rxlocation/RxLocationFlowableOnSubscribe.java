...
abstract class RxLocationFlowableOnSubscribe<T> extends RxLocationBaseOnSubscribe<T> implements FlowableOnSubscribe<T> {
    ...
    protected RxLocationFlowableOnSubscribe( RxLocation rxLocation, Long timeout, TimeUnit timeUnit) {
        super(rxLocation, timeout, timeUnit);
    }

    protected RxLocationFlowableOnSubscribe( Context ctx,  Api<? extends Api.ApiOptions.NotRequiredOptions>[] services, Scope[] scopes) {
